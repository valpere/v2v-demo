# Knowledge base — "Garant-Avto" car service / СТО (fictional, demo only)

All names, prices, and policies below are invented for a demo. Not a real
company. Figures are plausible for the Kyiv market (2026) but not taken from
any real service station. Any resemblance to an existing company called
«Гарант-Авто» is coincidental.

Garant-Avto is a multi-brand independent car service (СТО) in Kyiv for
private owners and companies: mechanical repair, scheduled maintenance,
diagnostics, tyre service and light body work. Prices are in UAH (грн). This
assistant only takes service requests and answers from this document — it
never diagnoses a car remotely, never judges whether a car is safe to drive,
and never quotes a firm repair total. The estimate is given by a service
advisor (майстер-приймальник) after the car is inspected.

Усі назви, ціни та умови нижче — вигадані для демо. Це не реальна компанія.
Цифри правдоподібні для київського ринку (2026), але не взяті з жодного
конкретного сервісу. Будь-який збіг із наявною компанією «Гарант-Авто» —
випадковий. «Гарант-Авто» — незалежний мультибрендовий автосервіс
(СТО) у Києві для приватних власників і компаній: механічний ремонт, планове
ТО, діагностика, шиномонтаж і легкий кузовний ремонт. Ціни — у гривнях (грн).
Цей асистент лише приймає заявки й відповідає з цього документа — він не
діагностує авто дистанційно, не оцінює, чи безпечно на ньому їхати, і не
називає точну суму ремонту. Кошторис дає майстер-приймальник після огляду
авто.

## Services / Послуги

Diagnostics (computer/OBD, engine, running-gear on a lift, brakes, A/C,
pre-purchase); scheduled maintenance (ТО) by interval or mileage — oil and
filters, spark plugs, brake fluid, coolant, timing-belt service; engine and
ancillaries (timing belt/chain, water pump, alternator, starter, cooling,
gaskets, mounts); transmission and clutch (manual gearbox repair, clutch,
oil change in manual and classic automatic gearboxes); suspension and
steering (shocks, struts, arms, silent-blocks, ball joints, bearings, tie
rods, steering-rack replacement); brakes (pads, discs, drums, calipers,
lines, ABS sensors); electrics and electronics (wiring faults, lighting,
battery, sensors, fault-code clearing); air conditioning (regas, leak
search, compressor, cabin filter); tyre service and 3D wheel alignment
(сход-розвал), R13–R18 mounting and balancing, puncture repair, seasonal
changeover, tyre storage; body and paint (minor dents, panel replacement,
localised painting and polishing, bumper repair).

The service does **not** do: overhaul of CVT and robotised gearboxes
(DSG / S-tronic) — only oil change and basic diagnostics on those; chip
tuning and power upgrades; trucks over 3.5 t and buses; electric vehicles
(only tyre service and alignment); motorcycles; heavy accident repair
needing chassis straightening on a jig. For any of these the assistant hands
the request to a manager.

**Towing (евакуатор).** Garant-Avto does not run its own tow truck. A
service advisor can recommend a partner tow operator; the tow is arranged
with and paid to that operator, separately from the repair. A client who
needs a tow can still leave a service request — the advisor sorts the tow
out on the call-back.

Діагностика (комп'ютерна/OBD, двигуна, ходової на підйомнику, гальм,
кондиціонера, передпродажна); планове ТО за регламентом або пробігом — олива
й фільтри, свічки, гальмівна рідина, антифриз, обслуговування ГРМ; двигун і
навісне (ремінь/ланцюг ГРМ, помпа, генератор, стартер, охолодження,
прокладки, подушки); трансмісія і зчеплення (ремонт механічної КПП,
зчеплення, заміна оливи в механічних і класичних гідротрансформаторних
АКПП); підвіска і кермо (амортизатори, стійки, важелі, сайлентблоки, кульові
опори, підшипники, рульові тяги, заміна рейки); гальма (колодки, диски,
барабани, супорти, трубки, датчики ABS); електрика та електроніка (проводка,
освітлення, акумулятор, датчики, скидання помилок); кондиціонер (заправка,
пошук витоку, компресор, салонний фільтр); шиномонтаж і сход-розвал 3D, монтаж
і балансування R13–R18, ремонт проколів, сезонна переобувка, зберігання шин;
кузовний ремонт і фарбування (дрібні вм'ятини, заміна елементів, локальне
фарбування й полірування, ремонт бамперів).

Сервіс **не виконує**: капремонт варіаторів (CVT) і роботизованих коробок
(DSG / S-tronic) — на них лише заміна оливи й базова діагностика; чип-тюнінг і
збільшення потужності; вантажівки понад 3,5 т і автобуси; електромобілі (лише
шиномонтаж і розвал); мототехніку; складний кузовний ремонт із витягуванням
геометрії на стапелі. За будь-чим із цього асистент передає заявку менеджеру.

**Евакуатор.** «Гарант-Авто» не має власного евакуатора. Майстер-приймальник
може порадити партнера-евакуатора; евакуацію клієнт замовляє й оплачує
окремо, тому оператору, а не сервісу. Якщо потрібен евакуатор, заявку на
обслуговування все одно можна залишити — майстер організує евакуацію під час
дзвінка.

## Brands / Марки

Works with most European, Japanese and Korean makes; best set up (parts
catalogues, software, experience) for **Volkswagen, Skoda, Audi, BMW, Toyota,
Mazda, Renault, Nissan, Hyundai, Kia, Ford, Peugeot, Citroën**. Basic work
(maintenance, brakes, suspension, tyres) is taken on almost any passenger
car, but deep work is **referred elsewhere** for Land Rover / Jaguar,
Porsche, Maserati, full-size American pickups (RAM, GMC), rare right-hand-
drive imports, and electric vehicles. If the client's make is on the refer
list, the assistant still takes the request and a manager confirms.

Працює з більшістю європейських, японських і корейських марок; найкраще
укомплектований (каталоги запчастин, ПЗ, досвід) для **Volkswagen, Skoda,
Audi, BMW, Toyota, Mazda, Renault, Nissan, Hyundai, Kia, Ford, Peugeot,
Citroën**. Базові роботи (ТО, гальма, підвіска, шини) беремо майже на
будь-яке легкове авто, але складні роботи **направляємо до інших** по Land
Rover / Jaguar, Porsche, Maserati, повнорозмірні американські пікапи (RAM,
GMC), рідкісні праворульні авто, електромобілі. Якщо марка клієнта в цьому
списку, асистент усе одно приймає заявку, а менеджер підтверджує.

## How pricing works / Як формується ціна

The price of a job is **labour + parts**; labour = the normo-hour rate times
the standard time for the operation.

- **Normo-hour (нормо-година): from 900 UAH.** Simple work is at the base
  rate; engine, gearbox, electronics and A/C work is 1 100–1 400 UAH/hour.
- **Diagnostics has a fee.** If the client then does the repair at
  Garant-Avto, the cost of the **basic** computer or running-gear diagnostics
  is credited toward the labour. In-depth diagnostics needing disassembly is
  not credited.
- **Parts markup:** on parts we order, the estimate shows the purchase price
  plus **10–25%**; every part is a separate line with its number.
- **Why a firm total needs an inspection:** the same symptom can have very
  different causes and cost. The assistant collects the details; the service
  advisor gives the estimate after seeing the car, never a firm total over
  chat.

Вартість роботи — це **робота + запчастини**; робота = нормо-година помножена
на нормативний час операції.

- **Нормо-година: від 900 грн.** Прості роботи — за базовою ставкою; двигун,
  КПП, електроніка й кондиціонер — 1 100–1 400 грн/год.
- **Діагностика платна.** Якщо клієнт після цього робить ремонт у
  «Гарант-Авто», вартість **базової** комп'ютерної діагностики або
  діагностики ходової зараховується у вартість робіт. Поглиблена діагностика
  з розбиранням не зараховується.
- **Націнка на запчастини:** на деталі, які замовляємо ми, у кошторисі —
  закупівельна ціна плюс **10–25%**; кожна деталь окремим рядком із номером.
- **Чому точна сума потребує огляду:** той самий симптом може мати різні
  причини й дуже різну вартість. Асистент збирає деталі; майстер-приймальник
  дає кошторис після огляду авто, а не в переписці.

## Indicative prices / Орієнтовні ціни (грн — a service advisor confirms after inspection)

Labour only unless stated; parts are extra and listed separately. Ranges are
for typical cars — SUVs, premium models and seized / rusted parts cost more.
The assistant quotes the range, not a single number.

- Computer (OBD) diagnostics / комп'ютерна діагностика: **600–1 200**
- Running-gear diagnostics on a lift / діагностика ходової: **500–900**
- Oil + oil filter change, labour / заміна оливи + фільтр, робота: **400–800**
  (oil and filter extra / олива й фільтр окремо)
- Brake pads, one axle, labour / гальмівні колодки, вісь, робота: **700–1 400**
- Timing-belt service, labour / заміна ременя ГРМ, робота: **3 500–9 000**
- Tyre service R13–R18, set of 4 / шиномонтаж R13–R18, комплект: **1 200–2 600**
- 3D wheel alignment / сход-розвал 3D (passenger car / легкове): **900–1 800**
- Normo-hour / нормо-година: **from 900 / від 900**

## Client-supplied parts / Запчастини клієнта

Garant-Avto **will install parts the client brings** if they are **new**, in
the maker's packaging, and match the car by catalogue number. Used parts are
not installed. Consumables (oil, filters, coolant, brake fluid) are accepted
but ours are recommended — the labour guarantee on that job depends on the
fluid meeting spec. The **labour guarantee still applies to our work** (the
installation), but **not** to the part itself or any failure it causes. If
the client's part turns out wrong or defective when we try to fit it, the
mechanic's time is still charged. Client-supplied parts are listed on the
intake sheet.

«Гарант-Авто» **встановлює запчастини клієнта**, якщо вони **нові**, у
заводській упаковці й підходять по каталожному номеру. Вживані деталі не
встановлюємо. Витратні матеріали (олива, фільтри, антифриз, гальмівна рідина)
приймаємо, але рекомендуємо свої — гарантія на цю роботу залежить від
відповідності рідини допуску. **Гарантія на нашу роботу (встановлення)
зберігається**, але **не** поширюється на саму деталь і на несправність,
спричинену нею. Якщо деталь клієнта виявиться неправильною або бракованою під
час встановлення, час майстра однаково оплачується. Запчастини клієнта
фіксуємо в акті приймання.

## Estimates and approval / Кошторис і погодження робіт

A **written estimate** with labour and parts as separate lines is prepared
before any work; work begins only after the client approves it. If more
problems are found mid-repair, the mechanic **stops, takes photos, and the
service advisor calls the client** to approve the extra work and cost
**before it is done** — nothing outside the approved estimate is done without
the client's agreement. On intake the car's **mileage, fuel level and
existing damage** (body and interior) are logged and photographed and the
client signs the intake sheet. After the work the client gets an act of
completed work (акт виконаних робіт) listing everything done and every part
used.

**Письмовий кошторис** із роботою й запчастинами окремими рядками готується
до початку робіт; роботи починаються лише після погодження клієнтом. Якщо в
процесі виявлені додаткові несправності, майстер **зупиняється, фотографує, і
майстер-приймальник телефонує клієнту** для погодження додаткових робіт і
вартості **до їх виконання** — нічого поза погодженим кошторисом без згоди
клієнта не робиться. При прийманні фіксуємо й фотографуємо **пробіг, рівень
палива та наявні пошкодження** (кузов і салон), клієнт підписує акт
приймання. Після робіт клієнт отримує акт виконаних робіт із переліком усього
зробленого та всіх використаних запчастин.

## Turnaround / Терміни

Scheduled maintenance and brake pads are often **same day** if the car is
booked in for the morning; computer diagnostics **same day**, running-gear
diagnostics **1–2 hours**. Engine, suspension or transmission work needing
ordered parts takes **1–4 working days** depending on parts availability;
body and paint **from 2 days**. A **courtesy car** — a small number, by prior
arrangement, deposit and fuel on the client. Waiting area with Wi-Fi and
coffee; a metro station is a 5-minute walk.

Планове ТО й гальмівні колодки — часто **того ж дня**, якщо записати авто на
ранок; комп'ютерна діагностика **того ж дня**, діагностика ходової **1–2
години**. Ремонт двигуна, підвіски чи трансмісії із замовленням запчастин —
**1–4 робочі дні** залежно від наявності запчастин; кузовний ремонт і
фарбування **від 2 днів**. **Підмінне авто** — обмежена кількість, за
попередньою домовленістю, застава й паливо за клієнтом. Зона очікування з
Wi-Fi і кавою; станція метро за 5 хвилин пішки.

## Guarantees / Гарантії

- **Labour:** 6 months or 10 000 km, whichever comes first; on major unit
  work (engine, gearbox internals) 3 months or 5 000 km.
- **Parts supplied by Garant-Avto:** the maker's warranty, usually 12
  months; a defective part is replaced under it and we handle the claim.
- **The guarantee does not apply if:** another service later worked on the
  same repaired unit; the car was driven with a known fault we warned about;
  the car was in an accident; it was used in motorsport or as a taxi beyond
  what was agreed; non-spec fluids were added elsewhere; the failure was
  caused by a part the client supplied.

A guarantee claim is a **warranty dispute** — the assistant hands it to a
manager.

- **На роботи:** 6 місяців або 10 000 км, залежно від того, що настане
  раніше; на складні агрегатні роботи (двигун, нутрощі КПП) — 3 місяці або
  5 000 км.
- **На запчастини, поставлені «Гарант-Авто»:** гарантія виробника, зазвичай
  12 місяців; браковану деталь міняємо за нею, претензію оформлюємо ми.
- **Гарантія не діє, якщо:** інший сервіс згодом втручався у той самий
  відремонтований вузол; авто експлуатували з відомою несправністю, про яку
  попереджали; авто потрапило в ДТП; його використовували в моторспорті або
  в таксі понад обумовлене; деінде залили невідповідні рідини; несправність
  спричинена запчастиною клієнта.

Гарантійна претензія — це **гарантійний спір**, асистент передає її менеджеру.

## Payment / Оплата

Cash, card (Visa / Mastercard), Apple Pay / Google Pay, and bank transfer for
companies with or without VAT. Payment is **after the work is done and the
car is handed over**. Each order comes with an act of completed work and a
guarantee card. Parts ordered specially are **prepaid 100%** of the part
price.

Готівка, картка (Visa / Mastercard), Apple Pay / Google Pay, безготівковий
розрахунок для компаній із ПДВ або без. Оплата — **після виконання робіт і
видачі авто**. До кожного замовлення — акт виконаних робіт і гарантійний
талон. Запчастини на замовлення — **передоплата 100%** вартості деталі.

## Your car and your belongings / Ваше авто та речі в салоні

From the moment the intake sheet (заявка-наряд) is signed, the car on the
premises is the service's responsibility. **Take documents and valuables with
you** — the service is not responsible for items left in the cabin or boot.
Leave **one set of keys**; if the car has an alarm, immobiliser or wheel
lock, tell the service advisor and leave the key / code. Existing damage and
mileage are photographed on intake.

З моменту підписання заявки-наряду за авто на території відповідає сервіс.
**Документи й цінні речі забирайте з собою** — за майно, залишене в салоні чи
багажнику, сервіс відповідальності не несе. Залишайте **один комплект
ключів**; якщо на авто сигналізація, іммобілайзер чи секретка на колесах —
повідомте майстра-приймальника й залиште ключ / код. Наявні пошкодження й
пробіг фотографуємо при прийманні.

## Working hours / Робочі години

Service: **Mon–Fri 09:00–20:00, Sat 09:00–17:00, Sun closed.** Cars are
accepted for same-day work until 18:00 on weekdays. The assistant takes
requests 24/7; a service advisor calls back during working hours, or the next
working morning for requests left overnight or on Sunday.

Сервіс: **Пн–Пт 09:00–20:00, Сб 09:00–17:00, Нд — вихідний.** Авто на роботи
того ж дня приймаємо до 18:00 у будні. Асистент приймає заявки цілодобово;
майстер-приймальник передзвонює в робочі години, а для заявок, залишених
вночі чи в неділю, — наступного робочого ранку.

## What the assistant collects / Що асистент запитає

To pass a clear request to the service advisor: **the car** (make, model,
year, engine/fuel if mentioned); **the problem** in the client's own words
(a knock in the suspension, won't start, check-engine light on, scheduled
maintenance, replace the brake pads, tyre service, body work after an
accident); **type of service** (diagnostics / repair / maintenance / tyre
service / body work — usually clear from the problem); **preferred date** to
bring the car in; **a phone number** for the callback. The assistant does not
book a firm slot or give a firm total — the service advisor confirms both by
phone.

Щоб передати чіткий запит майстру-приймальнику: **авто** (марка, модель, рік,
двигун/пальне, якщо згадали); **проблема** словами клієнта (стукіт у
підвісці, не заводиться, горить «чек», планове ТО, поміняти гальмівні
колодки, шиномонтаж, кузов після ДТП); **тип обслуговування** (діагностика /
ремонт / ТО / шиномонтаж / кузовний ремонт — зазвичай зрозуміло з проблеми);
**бажана дата** приїзду; **номер телефону** для зворотного дзвінка. Асистент
не бронює точний час і не називає точну суму — і те, і те підтверджує
майстер-приймальник телефоном.

Клієнти зазвичай пишуть коротко: «хочу записатися на сервіс», «треба пройти
ТО», «пригнати авто на діагностику», «записати машину на ремонт», «поміняти
масло і фільтри», «переобути на зиму / на літо», «зробити розвал-сходження»,
«загорівся значок на панелі», «щось стукає / гуде / деренчить», «не
заводиться», «тече», «поміняти колодки / диски / амортизатори», «підготувати
до продажу», «кузовний ремонт після невеликого ДТП». Асистент розпізнає марку
й модель (Volkswagen, Skoda, Audi, BMW, Toyota, Mazda, Renault, Nissan,
Hyundai, Kia, Ford, Peugeot, Citroen тощо), рік і тип пального (бензин,
дизель, гібрид) із повідомлення й не перепитує те, що вже сказали.

Clients usually write briefly: "book my car in", "I need a service / an MOT",
"bring it in for diagnostics", "oil and filter change", "swap to winter /
summer tyres", "wheel alignment", "warning light on the dash", "something is
knocking / humming / rattling", "won't start", "there's a leak", "new brake
pads / discs / shocks", "pre-sale check", "body repair after a small bump".
The assistant picks the make, model, year and fuel type out of the message
and does not re-ask what was already said.

## Common questions / Поширені запитання

- **How much is diagnostics?** — computer 600–1 200 UAH, running-gear
  500–900 UAH; depends on the car.
- **Is diagnostics credited toward the repair?** — yes, basic computer or
  running-gear diagnostics is credited toward the labour if the repair is
  done with us; in-depth diagnostics with disassembly is not.
- **Can I bring my own parts?** — yes, if new, in the maker's packaging,
  correct by catalogue number; we guarantee our work, not the part, and
  fitting time is charged if the part is wrong.
- **What is the guarantee on the work?** — 6 months or 10 000 km on labour
  (3 months / 5 000 km on major unit work); supplied parts carry the maker's
  warranty, usually 12 months.
- **Do you do wheel alignment?** — yes, 3D alignment for passenger cars,
  900–1 800 UAH.
- **Do you work with my make?** — most European, Japanese and Korean makes;
  some are referred elsewhere for deep work and a manager confirms.
- **Is there a courtesy car?** — a few, by prior arrangement, deposit and
  fuel on the client.
- **Do you have a tow truck?** — no own евакуатор; the advisor can recommend
  a partner tow operator, paid separately. Leave a service request anyway
  and the advisor arranges the tow on the call-back.
- **Can I book a specific time?** — leave a preferred date; the service
  advisor calls back to confirm the actual slot and the estimate. The
  assistant does not hold a firm time.
- **How do I book?** — give the assistant the car, the problem, a preferred
  date and a phone number; the service advisor calls back with an estimate
  and a slot.
- **Is it safe to drive my car like this?** — the service does not judge
  that over chat; an inspection is required.

Поширені запитання:

- **Скільки коштує діагностика?** — комп'ютерна 600–1 200 грн, ходової
  500–900 грн; залежить від авто.
- **Чи зараховується діагностика в ремонт?** — так, базова комп'ютерна
  діагностика або діагностика ходової зараховується у вартість робіт, якщо
  ремонт роблять у нас; поглиблена діагностика з розбиранням — ні.
- **Чи можна зі своїми запчастинами?** — так, якщо нові, у заводській
  упаковці, підходять по каталожному номеру; гарантуємо свою роботу, а не
  деталь, і час встановлення оплачується, якщо деталь неправильна.
- **Яка гарантія на роботу?** — 6 місяців або 10 000 км на роботи (3 місяці
  / 5 000 км на складні агрегатні); поставлені нами запчастини — гарантія
  виробника, зазвичай 12 місяців.
- **Чи робите розвал-сходження?** — так, 3D сход-розвал для легкових,
  900–1 800 грн.
- **Працюєте з моєю маркою?** — більшість європейських, японських і
  корейських марок; деякі щодо складних робіт направляємо до інших, менеджер
  підтверджує.
- **Чи є підмінне авто?** — кілька, за попередньою домовленістю, застава й
  паливо за клієнтом.
- **Чи є у вас евакуатор?** — власного евакуатора немає; майстер може
  порадити партнера-евакуатора, оплата окремо. Заявку на обслуговування все
  одно залиште — майстер організує евакуацію під час дзвінка.
- **Чи можна записатися на конкретний час?** — залиште бажану дату;
  майстер-приймальник передзвонить і підтвердить час та кошторис. Асистент
  точний час не бронює.
- **Як записатися?** — назвіть асистенту авто, проблему, бажану дату й номер
  телефону; майстер-приймальник передзвонить із кошторисом і слотом.
- **Чи безпечно так їхати?** — сервіс цього в переписці не оцінює; потрібен
  огляд.

## Escalate to a human when / Коли передавати менеджеру

Hand off to a manager when:

- the client asks whether the car is safe to drive, or wants any remote
  judgement about the car's condition — an inspection is required;
- a warranty dispute, or any claim about work Garant-Avto did;
- an insurance or accident claim, or work involving an insurer;
- a complaint about the service, staff, price or timing;
- a service the shop does not offer (CVT / DSG overhaul, chip tuning, trucks
  and buses, electric vehicles, motorcycles, heavy body / chassis work);
- a make on the refer-elsewhere list for deep work;
- the client asks for a person, or is unhappy with the assistant;
- anything not covered above.

Передавати менеджеру, коли:

- клієнт питає, чи безпечно їхати, або хоче дистанційної оцінки стану авто —
  потрібен огляд;
- гарантійний спір або претензія щодо робіт, які виконав «Гарант-Авто»;
- страховий випадок або ДТП, робота із залученням страхової;
- скарга на сервіс, персонал, ціну чи терміни;
- послуга, якої сервіс не надає (капремонт CVT / DSG, чип-тюнінг, вантажівки
  й автобуси, електромобілі, мототехніка, складний кузов / геометрія);
- марка зі списку тих, кого щодо складних робіт направляємо до інших;
- клієнт просить людину або незадоволений роботою асистента;
- будь-що, не описане вище.
