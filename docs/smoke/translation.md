# Smoke test — translation bureau (Бюро перекладів «FromToBridge» / Віра)

The per-topic scenario sweep for the **translation** assistant. The shared
...
scriptable with `minions/tgdrive/`). Only these need a real **voice
message**: scenario 1d, 12f / 12g, and all of section 13.

`[R]` = a regression case for a bug already found and fixed — those must keep
passing. Cross-references use the section number ("scenario 2a" = the first
block under **2. Quote flow**).

**Full reset** = `/reset` **and** clear the logs (`rm -f data/*.jsonl` with
the bot stopped) — section 2 and every step that checks a `leads.jsonl` row
count needs one before each lettered scenario. `/reset` alone (session only)
is enough elsewhere.
...
### 6a — should ANSWER (check the number against `topics/translation/kb.md`)
