...
| `internal/stt` | `Transcriber` interface, three impls: **`local`** (shell out to the `openai-whisper` CLI, which decodes the ogg via its own ffmpeg call; model name from `WHISPER_MODEL`, default `turbo` (= large-v3-turbo — faster than `medium` on CPU + better Ukrainian) — dev default, free), **`openai`** (`whisper-1` API — mandatory for the I-10 client recording, CPU-local is too slow live), and **`whispercpp`** (whisper.cpp's `whisper-cli` binary + a downloaded ggml model — no Python/PyTorch, opt-in only; converts OGG→WAV via `ffmpeg` first since its own decoder can't read Opus directly). `STT_BACKEND` selects; **`none`** yields a nil transcriber and a voice message gets a fixed decline reply, no download/transcribe attempted (symmetric with `TTS_BACKEND=none` below). | FR-2, D-13 |
...
| `internal/tts` | `Synthesizer` interface, three impls: **`elevenlabs`** (`eleven_multilingual_v2`) and **`azure`** (`uk-UA-*Neural`), both retrying once on a transient and emitting opus-in-ogg; **`espeak`** (espeak-ng formant synthesis + `ffmpeg` transcode — free, local, robotic, a `TTS_FALLBACK_BACKEND` "spare tire" not a primary). `TTS_BACKEND` selects; **`none`** yields a nil synthesizer and the update loop replies text-only (dev / bulk smoke-testing). Google is a documented fourth impl, not built (D-15). | FR-10, NFR-1, D-15 |
