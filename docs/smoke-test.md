...
1. `/reset`, then ask by voice: "Скільки коштує доставка кур'єром за
   ...
   - **Expect:** the reply names **DHL** and payment methods incl.
     **Privat24**. The **voice note** says "ді-ейч-ель" / "приват двадцять
   ...
   - **2026-09-03:** dialogue clean (DHL "at the carrier's tariff", all
     ...
     fixed in `tts.Spoken` (`DHL` → "ді ейч ель", same as ПДВ / Privat24).
...
2. Send a voice message, e.g. "Скільки коштує переклад паспорта на
   англійську?"
   - **Expect:** the voice note comes back in the **second** voice
...
1. `/reset`, then paste this as **one** message:

   > Добрий день, вибачте що так плутано пишу, у мене сьогодні голова обертом
   > і я вже кілька тижнів не можу зібратися з думками через переїзд, ми з
   > чоловіком нарешті вирішили що донька поїде вчитися за кордон, довго
   > обирали між кількома країнами, дивилися відгуки, рахували витрати на
   > гуртожиток, і зрештою зупинилися на Польщі бо там і ближче, і є знайомі
   > які допоможуть перші місяці. Загалом університет у Кракові попросив
   > офіційний переклад її диплома бакалавра з додатком, це десь три сторінки
   > якщо рахувати з оцінками, мова оригіналу українська, треба польською, і
   > бажано щоб було засвідчено печаткою бюро бо приймальна комісія на цьому
   > наполягає. Часу обмаль, документи від нас чекають до кінця місяця, після
   > того подача закривається і доведеться чекати наступного набору аж на рік,
   > чого дуже не хотілося б. Результат зручніше отримати сканом на пошту, бо
   > паперовий оригінал ми потім і так повеземо з собою. Ще думала спитати про
   > переклад свідоцтва про народження але то вже окрема історія, спершу
   > розберемося з дипломом. Скільки це буде коштувати і скільки днів займе?

   - **Expect:** handled, no error. It pulls what it can —
     `language_pair` укр→пол, `doc_type` диплом (з додатком),
     `volume` ~3 сторінки, `deadline` до кінця місяця,
