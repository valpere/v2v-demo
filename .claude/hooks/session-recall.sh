...
# accurately; measured on $CONTEXT alone, before the meter line itself.
CTX_BYTES=$(printf '%s' "$CONTEXT" | wc -c)
CTX_KB=$(awk -v b="$CTX_BYTES" 'BEGIN { printf "%.1f", b / 1000 }')

echo "[$(date -Iseconds)] session-recall: injecting context (${CTX_KB}kB)" >> "$LOG_FILE"

jq -n --arg ctx "$CONTEXT" --arg meter "_context injected: ${CTX_KB}kB_" '{
  hookSpecificOutput: {
