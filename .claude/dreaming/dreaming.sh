#!/bin/bash
# v2v-demo project dreaming pass.
#
# Purpose: scheduled (weekly) curation of THIS project only — AGENTS.md
# drift, .agents/plan.md package-layout invariant, stale memory,
# session-persistence scope creep, recurring commit themes. Read-only —
# outputs a report. Scoped to v2v-demo alone (never cross-mines other
# projects).
#
# Schedule: systemd user timer (Sun 08:00 nominal, Persistent=true catches
# up at next login).
...
echo "[$(date -Iseconds)] v2v-demo dreaming pass started" >> "$LOG"
...
echo "[$(date -Iseconds)] v2v-demo dreaming finished (exit=$EXIT, report=$REPORT)" >> "$LOG"
